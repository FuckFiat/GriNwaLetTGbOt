...
TOKEN = "1025641888:AAEKYEkg-QVtKImtzBgIFL03ahUIOnkRemo"
password = 'Zalupa3423'
admin_id = [771844687, 000000]
grin_pass_api = 'zrjvVfs6GH2BYgzgbR8Z'
storage = MemoryStorage()
bot = Bot(token=TOKEN)
dp = Dispatcher(bot, storage=storage)
grin_pass_api_v2 = '2OaE4XfxAcjtJdfcLkGv'
...
button5 = KeyboardButton('Balance')
button8 = KeyboardButton('TXS')
button9 = KeyboardButton('Cancel transaction')
markup3 = ReplyKeyboardMarkup().row(button1, button2)
markup3.row(button3, button4)
markup3.row(button5)
markup3.row(button6)
markup3.row(button7)
markup3.row(button8, button9)

@dp.message_handler(text='TXS') 
async def process_start_command(message: types.Message):
    if message['from']['id'] in admin_id:
        r = requests.post('http://127.0.0.1:3420/v2/owner', json = {"jsonrpc": "2.0", "method": "retrieve_txs", "params": [True, None, None], "id": 1}, auth=HTTPBasicAuth('grin', grin_pass_api_v2))
        send_last_trans_user = []
        last_trans = r.json()['result']['Ok'][-1]
        last_trans.reverse()
        for i in enumerate(last_trans):
            if i[0] == 3:
                break
            else:
                send_last_trans_user.append(i[1])
        send_last_trans_user.reverse()
        info_for_users = ''
        for i in send_last_trans_user:
            info_for_users = info_for_users + '\nAmount credited: ' + str(i['amount_credited']) + '\nAmount debited: ' + str(i['amount_debited']) + '\nConfirmation ts: ' + str(i['confirmation_ts']) + '\nConfirmed: ' + str(i['confirmed']) + '\nCreation ts: ' + str(i['creation_ts']) + '\nId: ' + str(i['id']) + '\nType: ' + str(i['tx_type']) + '\n======================================'
        await message.reply('```' + info_for_users + '```', reply=False, parse_mode=ParseMode.MARKDOWN)
...
# ExMachine Машина состояний для cancel


class Form_two(StatesGroup):
    id = State()  
    yes_no = State()



@dp.message_handler(text='Cancel transaction')
async def cmd_start(message: types.Message):
    if message['from']['id'] in admin_id:
        await Form_two.id.set()
        button1 = KeyboardButton('Cancel')
        markup2 = ReplyKeyboardMarkup().row(button1)
        await message.reply("(you can get it using the button TXS)\nInput id transaction::", reply=False, reply_markup=markup2)
    else:
        await message.reply('Permissions denied' ,parse_mode=ParseMode.MARKDOWN, reply=False)
...
@dp.message_handler(state=Form_two.id)
async def process_name(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await message.reply('Cancel transaction with id ' + data['id'] + ' ?', reply_markup=markup4)
        await Form_two.next()


@dp.message_handler(state=Form_two.yes_no)
async def yes_no(message: types.Message, state: FSMContext):
    await state.update_data(yes_no=message.text)
    if message['from']['id'] in admin_id:
        async with state.proxy() as data:
            if data['yes_no'] == 'Yes':
                info = os.popen(f'grin-wallet -p {password} ' + 'cancel -i ' + data['id']).read()
                await bot.send_message( message.chat.id, info, reply_markup=markup3)
            else:
                await bot.send_message( message.chat.id, 'Canceled.', reply_markup=markup3)
        await state.finish()
